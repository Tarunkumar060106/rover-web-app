...
const App = () => {
  ...
  useEffect(() => {
    const fetchSensorData = async () => {
      try {
        ...
        const response = await fetch('http://192.168.1.10:8000/rover/sensor_data'); // Replace with your Raspberry Pi's IP and HTTP endpoint
  ...
  const sendDirectionToRaspberryPi = async (angle: number | 'stop') => {
    ...
    try {
      ...
      await fetch('http://192.168.1.10:8000/rover/control', {
        method: 'POST',
        headers: {
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-8 py-8 grid gap-8 lg:grid-cols-5">
        ...
        <div className="bg-black border border-gray-600 overflow-hidden rounded-lg shadow-lg row-span-3 col-span-3 aspect-[4/3] relative">
          <iframe
            ...
            src="http://192.168.1.10:8000/video_feed" // Update video feed URL to Raspberry Pi's IP
            width="100%"
            height="100%"
